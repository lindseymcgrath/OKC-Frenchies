...
export const MarketingSidebar: React.FC<MarketingSidebarProps> = ({ 
    ...
    // For Mobile Drawer Logic
    const [activeDrawer, setActiveDrawer] = useState<string | null>(null);

    // Sync activeAccordion with activeDrawer on mobile to reuse logic/state where possible, 
    // but we usually want discrete control on mobile.
    
    // 🔥 AUTO-OPEN TRANSFORM DRAWER ON MOBILE IF LAYER SELECTED
    useEffect(() => {
        if (isMobile && studio.selectedLayer && activeDrawer !== 'transform') {
            setActiveDrawer('transform');
        }
    }, [studio.selectedLayer, isMobile]);
    ...
    // Helper to get current style for selected layer
    const currentStyle = studio.selectedLayer && studio.textStyles[studio.selectedLayer] 
        ? studio.textStyles[studio.selectedLayer] 
        : null;

    // --- REUSABLE COMPONENT: TRANSFORM EDITOR ---
    const renderTransformEditor = () => (
        <div className="bg-indigo-900/20 border border-indigo-500/50 p-3 rounded-sm animate-in zoom-in-95 h-full overflow-y-auto">
            <div className="flex justify-between items-center mb-4 border-b border-indigo-500/30 pb-2">
                <span className="text-[10px] font-bold uppercase text-indigo-400">Editing: {studio.selectedLayer}</span>
                <div className="flex gap-2">
                     <button onClick={studio.snapToCenter} className="text-indigo-400 hover:text-white bg-indigo-900/40 p-1 rounded" title="Snap to Center"><AlignCenter size={14}/></button>
                     {studio.selectedLayer.startsWith('sticker-') && (
                        <button onClick={() => studio.removeSticker(studio.selectedLayer)} className="text-red-400 hover:text-red-300 bg-red-900/20 px-2 py-1 rounded text-[9px] border border-red-900/50 flex items-center gap-1">
                            <Trash2 size={12}/> Delete
                        </button>
                    )}
                     <button 
                        onClick={() => { studio.setSelectedLayer(null); setActiveDrawer(null); }} 
                        className="bg-indigo-600 text-white px-3 py-1 rounded text-[9px] font-bold uppercase flex items-center gap-1"
                    >
                        <Check size={12} /> Done
                    </button>
                </div>
            ...
            <div className="space-y-6">
                <div>
                    <div className="flex justify-between mb-2">
                        <span className="text-[9px] uppercase text-slate-400 font-bold flex items-center gap-1"><RotateCw size={10}/> Rotate</span>
                        <span className="text-[9px] font-mono text-indigo-400">{studio.layerTransforms[studio.selectedLayer]?.rotate || 0}°</span>
                    </div>
                    <input 
                        type="range" 
                        min="0" 
                        max="360" 
                        value={studio.layerTransforms[studio.selectedLayer]?.rotate || 0} 
                        onChange={(e) => studio.updateTransform('rotate', parseInt(e.target.value))} 
                ...
                <div>
                    <div className="flex justify-between mb-2">
                        <span className="text-[9px] uppercase text-slate-400 font-bold flex items-center gap-1"><Scaling size={10}/> Scale</span>
                        <span className="text-[9px] font-mono text-indigo-400">{(studio.layerTransforms[studio.selectedLayer]?.scale || 1).toFixed(1)}x</span>
                    </div>
                    <input 
                        type="range" 
                        min="0.1" 
                        max="3.0" 
                        step="0.1" 
                        value={studio.layerTransforms[studio.selectedLayer]?.scale || 1} 
                        onChange={(e) => studio.updateTransform('scale', parseFloat(e.target.value))} 
                        className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                    />
                </div>
            </div>
    ...
    // --- REUSABLE COMPONENT: ASSETS PANEL ---
         ...
         <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
                <div className="flex flex-col gap-1">
                    ...
                    {studio.sireImage && (
                        <button onClick={() => handleProtectedAction(() => studio.handleBgRemoval('sire'))} disabled={studio.isProcessingImage} className="w-full py-2 text-[9px] bg-slate-800 hover:bg-slate-700 text-white uppercase font-bold rounded-sm border border-slate-700 flex items-center justify-center gap-1">
                            {studio.isProcessingImage && studio.processingType === 'sire' ? <Loader2 size={10} className="animate-spin"/> : <Scissors size={10}/>} Remove BG
                        </button>
                    )}
                </div>
                <div className="flex flex-col gap-1">
                    ...
                    {studio.damImage && (
                        <button onClick={() => handleProtectedAction(() => studio.handleBgRemoval('dam'))} disabled={studio.isProcessingImage} className="w-full py-2 text-[9px] bg-slate-800 hover:bg-slate-700 text-white uppercase font-bold rounded-sm border border-slate-700 flex items-center justify-center gap-1">
                            {studio.isProcessingImage && studio.processingType === 'dam' ? <Loader2 size={10} className="animate-spin"/> : <Scissors size={10}/>} Remove BG
                        </button>
                    )}
                </div>
            </div>
    ...
    // --- REUSABLE COMPONENT: LAYOUT PANEL ---
    const renderLayoutPanel = () => (
         <div className="space-y-4">
            <div className="flex gap-2 justify-center pb-2 border-b border-slate-800">
                <button onClick={() => studio.changeAspectRatio('1:1')} className={`p-2 rounded border transition-all ${studio.aspectRatio==='1:1' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-black text-slate-500 border-slate-700'}`}><Grid3X3 size={16}/></button>
                <button onClick={() => studio.changeAspectRatio('4:5')} className={`p-2 rounded border transition-all ${studio.aspectRatio==='4:5' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-black text-slate-500 border-slate-700'}`}><RectangleVertical size={16}/></button>
                <button onClick={() => studio.changeAspectRatio('9:16')} className={`p-2 rounded border transition-all ${studio.aspectRatio==='9:16' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-black text-slate-500 border-slate-700'}`}><Smartphone size={16}/></button>
            </div>
            ...
            <div>
                ...
                <textarea 
                    ...
                    className="w-full bg-black/40 border border-slate-700 p-2 text-[10px] text-white h-24 resize-none focus:border-indigo-500 outline-none rounded-sm" 
                    placeholder="Describe your scene..." 
                />
            </div>
            
            <button onClick={() => handleProtectedAction(studio.handleGenerateScene)} disabled={studio.isGeneratingScene} className="w-full px-4 py-3 bg-indigo-600 text-white text-[10px] uppercase font-bold rounded-sm flex items-center justify-center gap-2 hover:bg-indigo-500 shadow-lg disabled:opacity-50">
                {studio.isGeneratingScene ? <Loader2 className="animate-spin" size={12}/> : <><Sparkles size={12}/> Generate Background</>}
            </button>
        </div>
    );

    // --- REUSABLE COMPONENT: TEXT EDITOR ---
    const renderTextEditor = () => (
        <div className="space-y-4">
             {/* Text Style Controls (Only show if Text Layer Selected) */}
             {currentStyle && (
                <div className="bg-indigo-900/10 border border-indigo-500/30 p-2 rounded-sm space-y-3 animate-in fade-in">
                    <div className="flex items-center justify-between mb-1 border-b border-white/5 pb-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-bold uppercase text-indigo-300">Style: {studio.selectedLayer}</span>
                        </div>
                    </div>
                    
                    {/* Font Selection */}
                    <div className="grid grid-cols-4 gap-1">
                        {['Cinzel', 'Manrope', 'Playfair Display', 'Cursive'].map(font => (
                            <button
                                key={font}
                                onClick={() => studio.updateSelectedStyle('font', font)}
                                className={`py-1 text-[8px] rounded-sm border ${currentStyle.font === font ? 'bg-indigo-500 text-white border-indigo-400' : 'bg-black/40 text-slate-500 border-slate-800'}`}
                                style={{ fontFamily: font }}
                            >
                                {font === 'Cursive' ? 'Script' : font.split(' ')[0]}
                            </button>
                        ))}
                    </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800">
                {['header', 'studName', 'damName', 'studPheno', 'studGeno'].map(layer => {
                    let label = layer;
                    let val = '';
                    let setter: any = null;
                    let visible = true;
                    ...
                    if (layer === 'header') { label = 'Header'; val = studio.headerText; setter = studio.setHeaderText; visible = studio.showHeader; toggler = studio.setShowHeader; }
                    if (layer === 'studName') { label = 'Sire Name'; val = studio.studName; setter = studio.setStudName; visible = studio.showStudName; toggler = studio.setShowStudName; }
                    if (layer === 'damName') { label = 'Dam Name'; val = studio.damName; setter = studio.setDamName; visible = studio.showDamName; toggler = studio.setShowDamName; }
                    if (layer === 'studPheno') { label = 'Phenotype'; val = studio.studPhenotype; setter = studio.setStudPhenotype; visible = studio.showPhenotype; toggler = studio.setShowPhenotype; }
                    if (layer === 'studGeno') { label = 'Genotype'; val = studio.studDna; setter = studio.setStudDna; visible = studio.showGenotype; toggler = studio.setShowGenotype; }

                    return (
                        <div key={layer} className="flex items-center gap-2 bg-black/20 p-2 rounded border border-slate-800">
                            <button onClick={() => toggler(!visible)} className="hover:scale-110 transition-transform">
                                {visible ? <ToggleRight size={14} className="text-emerald-400"/> : <ToggleLeft size={14} className="text-slate-500"/>}
                            </button>
                            <input 
                                value={val} 
                                onChange={(e) => setter(e.target.value)} 
                                onClick={() => studio.setSelectedLayer(layer)}
                                className={`flex-1 bg-transparent text-[10px] outline-none placeholder-slate-600 ${studio.selectedLayer === layer ? 'text-indigo-400 font-bold' : 'text-slate-300'}`} 
                                placeholder={label}
                            />
                        </div>
                    );
    ...
    // --- REUSABLE COMPONENT: STICKER PANEL ---
    ...
    // ===========================================
    // MOBILE LAYOUT (CANVA STYLE DOCK)
    // ===========================================
    if (isMobile) {
        return (
            <>
                {/* Fixed Bottom Dock */}
                <div className="fixed bottom-0 left-0 w-full bg-[#0a0a0a] border-t border-slate-800 z-[60] flex flex-col shadow-[0_-5px_20px_rgba(0,0,0,0.8)] pb-safe">
                    ...
                    <div className="flex justify-around items-center py-3">
                        <button 
                            ...
                            className={`flex flex-col items-center gap-1 ${activeDrawer === 'assets' ? 'text-white' : 'text-slate-500'}`}
                        >
                            <Briefcase size={20} />
                            <span className="text-[8px] uppercase font-bold">Assets</span>
                        </button>
                        <button 
                            onClick={() => setActiveDrawer(activeDrawer === 'layout' ? null : 'layout')} 
                            className={`flex flex-col items-center gap-1 ${activeDrawer === 'layout' ? 'text-white' : 'text-slate-500'}`}
                        >
                            <Wand2 size={20} />
                            <span className="text-[8px] uppercase font-bold">Scene</span>
                        </button>
                        
                        {/* Editor Icon Highlighted */}
                        <button 
                            onClick={() => setActiveDrawer(activeDrawer === 'text' ? null : 'text')} 
                            className={`flex flex-col items-center gap-1 p-2 -mt-6 bg-slate-800 rounded-full border border-slate-700 shadow-lg ${activeDrawer === 'text' ? 'text-luxury-teal border-luxury-teal' : 'text-slate-300'}`}
                        >
                            <Layers size={24} />
                            <span className="text-[8px] uppercase font-bold mt-1">Editor</span>
                        </button>

                         <button 
                            onClick={() => setActiveDrawer(activeDrawer === 'stickers' ? null : 'stickers')} 
                            className={`flex flex-col items-center gap-1 ${activeDrawer === 'stickers' ? 'text-white' : 'text-slate-500'}`}
                        >
                            <StickerIcon size={20} />
                            <span className="text-[8px] uppercase font-bold">Stickers</span>
                        </button>
                    </div>
                </div>
                ...
                {activeDrawer && (
                    <div className="fixed bottom-[85px] left-0 w-full bg-[#0f172a]/95 backdrop-blur-xl border-t border-slate-700 rounded-t-xl z-[50] shadow-2xl animate-in slide-in-from-bottom-10 h-[45vh] flex flex-col">
                        <div className="flex justify-center pt-2 pb-1" onClick={() => setActiveDrawer(null)}>
                            <div className="w-12 h-1 bg-slate-600 rounded-full cursor-pointer opacity-50 hover:opacity-100"></div>
                        </div>
                        <div className="flex justify-between items-center px-4 py-2 border-b border-slate-800">
                            <span className="text-xs font-serif text-white uppercase tracking-widest">
                                {activeDrawer === 'assets' && "Project Assets"}
                                {activeDrawer === 'layout' && "Scene & Layout"}
                                {activeDrawer === 'text' && "Text & Layers"}
                                {activeDrawer === 'stickers' && "Stickers"}
                                {activeDrawer === 'transform' && "Transform"}
                            </span>
                            <button onClick={() => setActiveDrawer(null)}><Minimize2 size={16} className="text-slate-400"/></button>
                        </div>
                        
                        <div className="flex-1 overflow-y-auto p-4 pb-8">
                             {activeDrawer === 'assets' && renderAssetsPanel()}
                             {activeDrawer === 'layout' && renderLayoutPanel()}
                             {activeDrawer === 'text' && renderTextEditor()}
                             {activeDrawer === 'stickers' && renderStickerPanel()}
                             {activeDrawer === 'transform' && renderTransformEditor()}
                        </div>
                    </div>
                )}
            </>
    ...
    return (
        <div className="w-full lg:w-[360px] flex-shrink-0 flex flex-col gap-2 order-1 lg:order-2">
            ...
            <div className="mb-2 p-4 bg-gradient-to-br from-[#1a1a1a] to-black border border-luxury-gold/40 rounded-sm shadow-2xl">
                <div className="flex justify-between items-center mb-3">
                    <div className="flex flex-col">
                        ...
                        <span className="text-lg font-serif text-white">{statusText}</span>
                    </div>
                    {(!isPro) && (
                        <button onClick={() => setShowPaywall(true)} className="px-4 py-2 bg-luxury-gold hover:bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest rounded-sm transition-all shadow-[0_0_15px_rgba(212,175,55,0.2)]">
                ...
                {studio.isSessionActive && (
                     <div className="mb-3 px-2 py-1 bg-luxury-teal/10 border border-luxury-teal/20 rounded-sm flex items-center gap-2">
                        ...
                        <span className="text-[9px] text-luxury-teal font-bold uppercase tracking-wider">Session Unlocked</span>
                   </div>
                )}
            </div>
            ...
            <div className="order-2">
                <Accordion id="scene" title="Layout & Atmosphere" icon={Wand2} studio={studio}>
                    {renderLayoutPanel()}
                </Accordion>
            </div>
            ...
            {/* SELECTED TRANSFORM EDITOR (DESKTOP) */}
            {studio.selectedLayer && (
                <div className="order-5 mb-2 h-40">
            ...
            {/* ATMOSPHERE BROWSE MODAL (Shared) */}
